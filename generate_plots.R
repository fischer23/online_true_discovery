...
df_GRO <- TDP_df[TDP_df$Procedure %in% c("GRO", "hedged GRO", "true proportion"), ]
col_GRO <- c("darksalmon", "orange", "grey")
...
df_best <- TDP_df[TDP_df$Procedure %in% c("admissible online-simple", "hedged GRO", "true proportion"), ]
col_best <- c("cornflowerblue", "orange", "grey")
...
# Creates Figure S.3 of the paper "Admissible online closed testing must employ e-values"

df_boost <- TDP_df[TDP_df$Procedure %in% c("admissible online-simple", "boosted GRO", "calibrated", "true proportion"), ]
col_boost <- c("cornflowerblue", "orange", "limegreen", "grey")
...
plot_boost <- ggplot(df_boost, aes(x = index, y = TDP, colour = Procedure, linetype = Procedure)) +
  geom_line(size = 1.2, aes(colour = Procedure)) +
  facet_grid(mu_A ~ pi_A) +
  scale_colour_manual(values = col_boost) +
  scale_linetype_manual(values = line_types) +
  xlab("Hypothesis index") +
  ylab("TDP bound") +
  scale_x_continuous(breaks = seq(100, 900, 200), limits = c(0, 1000), expand = c(0, 0)) +
...
ggsave("results/plot_boost.pdf", plot = plot_boost, width = 12, height = 8)
